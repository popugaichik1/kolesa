...
// fakeUserRepo — ручная fake-реализация UserRepo. Каждый метод делегирует
// в поле-функцию, которое выставляет конкретный тест-кейс — так каждый тест
// сам решает, что должен вернуть репозиторий, без мок-фреймворка.
...
// fakePublisher по умолчанию (без заданного publishFunc) считает публикацию
// успешной — большинству тестов, которые не проверяют Kafka, это позволяет
// не задавать функцию вовсе.
